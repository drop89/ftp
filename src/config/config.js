...
// Receive messages content in webhook per type
const WEBHOOK_MESSAGE = !!(
    process.env.WEBHOOK_MESSAGE && process.env.WEBHOOK_MESSAGE === 'true'
)
const WEBHOOK_CONNECTION = !!(
    process.env.WEBHOOK_CONNECTION && process.env.WEBHOOK_CONNECTION === 'true'
)
const WEBHOOK_PRESENCE = !!(
    process.env.WEBHOOK_PRESENCE && process.env.WEBHOOK_PRESENCE === 'true'
)
const WEBHOOK_UPDATE = !!(
    process.env.WEBHOOK_UPDATE && process.env.WEBHOOK_UPDATE === 'true'
)
const WEBHOOK_CALL = !!(
    process.env.WEBHOOK_CALL && process.env.WEBHOOK_CALL === 'true'
)
const WEBHOOK_GROUP = !!(
    process.env.WEBHOOK_GROUP && process.env.WEBHOOK_GROUP === 'true'
)
...
module.exports = {
    ...
    webhookConnection: WEBHOOK_CONNECTION,
    webhookPresence: WEBHOOK_PRESENCE,
    webhookUpdate: WEBHOOK_UPDATE,
    webhookCall: WEBHOOK_CALL,
    webhookGroup: WEBHOOK_GROUP,
